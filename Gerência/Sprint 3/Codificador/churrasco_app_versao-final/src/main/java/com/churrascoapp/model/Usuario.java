...
public class Usuario {
    ...
    private String papel; // Usuário, Administrador

    public Usuario(){}

    public Usuario(UUID id, String nome, String email, String senha, String papel) {
    ...
    public String toCSV() {
        return String.join(";", id.toString(), nome, email, senha, papel);
    }

    public static Usuario fromCSV(String line) {
        String[] p = line.split(";", -1);
        if (p.length < 5) return null;
        return new Usuario(UUID.fromString(p[0]), p[1], p[2], p[3], p[4]);
    }
}
